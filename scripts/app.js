...
(function () {
  ...
  app.updateForecastCard = function (data) {
    ...
    card.querySelector('.current .humidity').textContent = Math.round(
      humidity
    ) + '%'
    ...
    for (var i = 0; i < 7; i++) {
      ...
      if (daily && nextDay) {
        nextDay.querySelector('.date').textContent = app.daysOfWeek[
          (i + today) % 7
        ]
        nextDay
          .querySelector('.icon')
  ...
  app.getForecast = function (key, label) {
    var statement = 'select * from weather.forecast where woeid=' +
      key +
      " and u='c'"
